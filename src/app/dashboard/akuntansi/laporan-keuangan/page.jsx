...
//     const percentageToShow = percentage !== null && typeof percentage === 'number';
...
//             // Panggilan API ke backend yang sebenarnya
...
//             const totalPemasukan = rawData.reduce((sum, item) => sum + parseFloat(item.total_pemasukan || 0), 0);
//             const totalPengeluaran = rawData.reduce((sum, item) => sum + parseFloat(item.total_pengeluaran || 0), 0);
            
//             const latestData = rawData[0];
//             const totalKas = parseFloat(latestData.net_cash || 0);
...
//             setSummaryCardData({ totalPemasukan, totalPengeluaran, totalKas, persenPemasukan, persenPengeluaran, marginKasBersih });
...
//             {chartData12Months.length > 0 && ` (${chartData12Months[0].label} - ${chartData12Months[chartData12Months.length-1].label})`}
//           </p>
...
//             Memuat data grafik...
//            </div>
//           ) : error || chartData12Months.length === 0 ? (
//             <div className="flex flex-col justify-center items-center h-[350px] text-slate-500">
//                 <BarChart2 className="w-10 h-10 text-slate-400 opacity-50 mb-2" />
//                 {error || "Tidak ada data untuk ditampilkan pada grafik."}
//             </div>
//           ) : (
//            <ResponsiveContainer width="100%" height={350}>
//             <LineChart data={chartData12Months} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
    ...
    const fetchFinancialData = useCallback(async () => {
        ...
        try {
            ...
            if (result.status !== 'success' || !Array.isArray(result.data) || result.data.length === 0) {
                throw new Error(result.message || "Data statistik tidak ditemukan untuk tahun ini.");
            }
            ...
            // Ambil `net_cash` dari bulan terakhir yang ada datanya
            // Controller sudah mengurutkan, jadi ambil saja item terakhir dari array.
            ...
            const transformedData = rawData.map(item => {
                ...
                return {
                    ...
                    pemasukan: parseFloat(item.total_pemasukan || 0),
                    pengeluaran: parseFloat(item.total_pengeluaran || 0),
                    kas: parseFloat(item.net_cash || 0), // Menggunakan net_cash langsung dari API
                };
            });
